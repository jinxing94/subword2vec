...
with graph2.as_default():
	...
	with tf.device('/cpu:0'):
		...
		concat_context = tf.concat(1,[word_context,char_context,comp_context])
		print(concat_context)
		nce_weights = tf.Variable(
			tf.truncated_normal([word_size, 3 * embedding_size],
				stddev=1.0 / math.sqrt(3 * embedding_size)))
		nce_biases = tf.Variable(tf.zeros([word_size]))
		loss = tf.reduce_mean(
			tf.nn.nce_loss(nce_weights, nce_biases, concat_context, tword_label,num_sampled, word_size))
