...
def train_thread(pid, start_index, end_index):
	...
	for sample_index in xrange(start_index, end_index):
		...
		loss1_val, loss2_val, loss3_val,loss_val,step,_ = session.run([loss1,loss2,loss3,loss,global_step,optimizer], feed_dict=feed_dict)
		#use the thread info to print the log and save the model at proper time
		if step > 0 and step % print_steps == 0:
			localtime = time.asctime(time.localtime(time.time()))
			print('time: ',localtime,' Thread ',pid, ' step ', step, ' loss ',loss_val,' loss1 ',loss1_val,' loss2 ',loss2_val,' loss3 ',loss3_val)
			sys.stdout.flush()
